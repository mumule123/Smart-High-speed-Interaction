...
class OperationDetector:
    def __init__(self, project_root, classes_file, operation_threshold, config):
        ...
        try:
            ...
            '''
                device_classes = {
                    2: "主断路器检查",           # 主断路器(ID=2) → 主断路器检查
                    4: "受电弓实验",             # 受电弓升起(ID=4) → 受电弓实验  
            '''
            self.device_classes = {}
        ...
        # 操作记录
        # 普通字典 (dict) 的子类，但有一个特殊功能：当访问不存在的键时，会自动创建该键并赋予默认值
    ...
    def process_video(self, video_path, output_dir):
        ...
        while True:
            ...
            if frame_count % 500 == 0:  # 每500帧显示进度
...
def main():
    ...
    parser.add_argument('--output', type=str,
                       default=os.path.join(project_root, 'analysis_resultsv2'),
                       help='输出结果目录路径')
